...
| 15 | `content.per_side BOOLEAN NOT NULL DEFAULT FALSE` | 24 movements say "per side" or "per leg" in prose only, so the app undercounts them. `bulgarian_split_squats` is 1 set of 6 with "per side" in its notes, and the player counts 6 when you owe 12. **Done 2026-09-08** |
